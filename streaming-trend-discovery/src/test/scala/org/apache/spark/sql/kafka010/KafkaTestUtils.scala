...
/**
  ...
  */
abstract class KafkaTestUtils[A, B](withBrokerProps: Map[String, Object] = Map.empty) extends Logging {

  def keySerializer(): Serializer[A]
  def valueSerializer(): Serializer[B]
  def keyDeserializer(): Deserializer[A]
  def valueDeserializer(): Deserializer[B]
  ...
  // Kafka producer
  private var producer: Producer[A, B] = _
  ...
  def sendMessages(topic: String, messages: Iterable[B], keyExtractor: (B) => A): Seq[(B, RecordMetadata)] = {
    sendMessages(topic, messages, keyExtractor, None, None)
  }

  /** Send the array of messages to the Kafka broker using specified partition */
  def sendMessages(topic: String, messages: Iterable[B], keyExtractor: (B) => A, timestampExtractor: Option[(B) => Long] = None,
                   partition: Option[Int] = None): Seq[(B, RecordMetadata)] = {
    producer = new KafkaProducer[A, B](producerConfiguration)
    val offsets = try {
      messages.map { m =>
        val key = keyExtractor(m)
        val record = partition match {
          case Some(p) =>
            if (timestampExtractor.isDefined) {
              new ProducerRecord[A, B](topic, p, timestampExtractor.get.apply(m), key, m)
            } else {
              new ProducerRecord[A, B](topic, p, key, m)
            }
          case None =>
            if (timestampExtractor.isDefined) {
              new ProducerRecord[A, B](topic, 0, timestampExtractor.get.apply(m), key, m)
            } else {
              new ProducerRecord[A, B](topic, key, m)
            }
        }
        val metadata =
          producer.send(record).get(10, TimeUnit.SECONDS)
        logDebug(s"\tSent $m to partition ${metadata.partition}, offset ${metadata.offset}")
        (m, metadata)
      }
    } finally {
      if (producer != null) {
  ...
  def getMessages[T <: Message](topic: String, valueDeserializerClassName: String, autoOffsetReset: String, brokers: Option[String]=None): ConsumerRecords[String, T] = {
    val consumerConfig = new Properties()
    consumerConfig.put("bootstrap.servers", brokers.getOrElse(brokerAddress))
    consumerConfig.put("group.id", "group-KafkaTestUtils-" + Random.nextInt)
    consumerConfig.put("key.deserializer", classOf[StringDeserializer].getName)
    consumerConfig.put("value.deserializer", valueDeserializerClassName)
    consumerConfig.put("auto.offset.reset", autoOffsetReset)
    consumerConfig.put("enable.auto.commit", "false")

    val kc = new KafkaConsumer[String, T](consumerConfig)
    kc.subscribe(Set(topic).asJavaCollection)
    kc.poll(0)
    val partitions = kc.assignment()
    kc.pause(partitions)
    kc.seekToBeginning(partitions)
    kc.resume(partitions)
    val records = kc.poll(10000)
    kc.close()
    records
  }
  ...
  def getEarliestOffsets(topics: Set[String]): Map[TopicPartition, Long] = {
    val kc = new KafkaConsumer[A, B](consumerConfiguration)
